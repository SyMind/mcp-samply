...
test("profile analysis can restore external symbols through atos and rustfilt", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "mcp-samply-symbolize-"));
  const profilePath = path.join(tempDir, "fixture.json");
  const fakeLibPath = path.join(tempDir, "fake.node");
  const fakeAtosPath = path.join(tempDir, "fake-atos.mjs");
  const fakeRustfiltPath = path.join(tempDir, "fake-rustfilt.mjs");
  const originalAtos = process.env.MCP_SAMPLY_ATOS_BIN;
  const originalRustfilt = process.env.MCP_SAMPLY_RUSTFILT_BIN;
  ...
  await writeFile(profilePath, JSON.stringify(profile), "utf8");
  await writeFile(fakeLibPath, "", "utf8");
  await writeFile(
    fakeAtosPath,
    [
      "#!/usr/bin/env node",
      ...
      "  if (address.toLowerCase() === '0x20') {",
      "    console.log('_RNvC6module4func (in fake.node) + 16');",
      "  } else {",
      "    console.log(address);",
      "  }",
      "}",
  ...
  await chmod(fakeAtosPath, 0o755);
  await chmod(fakeRustfiltPath, 0o755);

  process.env.MCP_SAMPLY_ATOS_BIN = fakeAtosPath;
  process.env.MCP_SAMPLY_RUSTFILT_BIN = fakeRustfiltPath;
  ...
  try {
    ...
    const search = searchFunctions(loadedProfile, "module::func", {
      maxResults: 5,
      maxThreadsPerResult: 5,
    });

    assert.equal(
      summary.hottestStackFunctionsOverall[0]?.name,
      "module::func (in fake.node) + 16",
    );
    assert.equal(search.matchCount, 1);
    assert.equal(search.matches[0]?.name, "module::func (in fake.node) + 16");
  } finally {
    if (originalAtos === undefined) {
      delete process.env.MCP_SAMPLY_ATOS_BIN;
    } else {
      process.env.MCP_SAMPLY_ATOS_BIN = originalAtos;
    }
    if (originalRustfilt === undefined) {
      delete process.env.MCP_SAMPLY_RUSTFILT_BIN;
    } else {
      process.env.MCP_SAMPLY_RUSTFILT_BIN = originalRustfilt;
    }
    await rm(tempDir, { recursive: true, force: true });
  }
});
